...
class CodeGen:
    ...
    def gen(self, vars_, funcs):
        # 1. Сначала регистрируем адреса ВСЕХ функций, чтобы вызовы могли ссылаться вперед
        for f in funcs:
            self.func_addresses[f.name] = 0
        ...
        # 3. Теперь генерируем код функций и обновляем их реальные адреса
        for f in funcs:
            self.current_func = f
            self.func_addresses[f.name] = len(self.code) # Установка реального адреса
            self.locals = {p: i for i, p in enumerate(f.params)}
            for stmt in f.body:
                self.gen_stmt(stmt)
            self.emit(22) # RETURN

        # Патчим прыжок в main
    ...
    def gen_stmt(self, s):
        if isinstance(s, VarDecl):
            ...
            elif name in self.globals: self.emit(4, self.globals[name])
    ...
    def gen_expr(self, e):
        if isinstance(e, Number):
            ...
            self.emit(1, [k for k, v in self.string_pool.items() if v == e.value][0])
        # ... (Var и BinOp без изменений) ...
        elif isinstance(e, Var):
            name = f"{self.current_func.name}_{e.name}" if self.current_func else e.name
            if e.name in self.locals:
                self.emit(5, self.locals[e.name])
            elif name in self.globals:
                self.emit(3, self.globals[name])
            else:
                self.emit(3, self.globals[e.name])
        elif isinstance(e, BinOp):
            self.gen_expr(e.left)
            self.gen_expr(e.right)
            # Добавьте "<<" в словарь ops
        ...
        # --- ИСПРАВЛЕНИЯ НИЖЕ ---
            ...
            self.emit(41)  # БЫЛО 40 -> СТАЛО 41 (Соответствует xvm.py: elif op == 41)

        elif isinstance(e, ArrayAccess):
            self.gen_expr(Var(e.name))
            self.gen_expr(e.index)
            self.emit(42)  # Верно (42 в xvm)

        elif isinstance(e, Call):
            # СИСТЕМНЫЕ ВЫЗОВЫ
            if e.name == "prints":
                self.gen_expr(e.args[0]);
                self.emit(45)  # БЫЛО 40 -> СТАЛО 45 (Соответствует xvm.py: elif op == 45)
            elif e.name == "printi":
                self.gen_expr(e.args[0]);
                self.emit(46)  # БЫЛО 41 -> СТАЛО 46 (41 занят Alloc, 46 это PRINTHEX в xvm)
            elif e.name == "fwrite":
                self.gen_expr(e.args[0]);
